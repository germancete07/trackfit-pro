...
export default async function ProgressPage() {
  ...
  return <ProgressPageContent studentId={user.id} />;
}

async function ProgressPageContent({ studentId }: { studentId: string }) {
  const supabase = await createClient();
  ...
  // RPE per session
  const sessionRpe: Record<string, { sum: number; count: number; date: string; name: string }> = {};
  for (const l of recent) {
    if (!l.rpe || !l.session_id) continue;
    if (!sessionRpe[l.session_id]) {
      sessionRpe[l.session_id] = { sum: 0, count: 0, date: l.logged_at, name: "" };
    }
    sessionRpe[l.session_id].sum += l.rpe;
    sessionRpe[l.session_id].count++;
  }
  const rpeTrend = Object.entries(sessionRpe)
    .map(([, v]) => ({ session_name: v.name, date: v.date, avg_rpe: Math.round((v.sum / v.count) * 10) / 10 }))
  ...
  // Weight history per exercise (max per week)
  ...
  return (
    <div className="px-4 py-5 flex flex-col gap-4">
      <h1 className="text-xl font-black text-gray-900">Mi progreso</h1>
      <ProgressView
        prs={prs}
