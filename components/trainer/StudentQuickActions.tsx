...
import { calcFixedMenuPos, type FixedMenuPos } from "@/lib/useFixedMenu";
...
export function StudentQuickActions({ studentId, studentName, hasActiveRoutine, unreadMessages, pendingVideos }: Props) {
  ...
  function handleToggle(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    setPos(calcFixedMenuPos(e.currentTarget.getBoundingClientRect(), 210, 280));
    setOpen(true);
  }
    ...
    <div className="relative flex-shrink-0">
      ...
      {open && (
        <>
          ...
          {/* Popover — fixed, never clipped */}
          <div
            onMouseDown={e => e.stopPropagation()}
            style={{ position: "fixed", top: pos.top, left: pos.left, zIndex: 9999, width: 210 }}
          >
            <div
              className="rounded-2xl shadow-xl py-2"
              style={{ background: "var(--surface-elevated, #fff)", border: "0.5px solid rgba(0,0,0,0.08)", boxShadow: "0 8px 32px rgba(0,0,0,0.16)" }}
            >
              <p className="px-3 py-1.5 text-xs font-bold text-gray-400 uppercase tracking-wide border-b border-gray-100 mb-1 truncate">
                {studentName}
              </p>

              {unreadMessages > 0 && (
                <button onClick={() => go("/dashboard/chat")} className={rowCls}>
                  <IconWrap color="blue"><ChatSvg /></IconWrap>
                  <div>
              ...
              {pendingVideos > 0 && (
                <button onClick={() => go(`/dashboard/students/${studentId}?tab=correcciones`)} className={rowCls}>
                  <IconWrap color="purple"><VideoSvg /></IconWrap>
                  <div>
              ...
              <button onClick={() => go(`/dashboard/students/${studentId}`)} className={rowCls}>
                <IconWrap color="brand"><UserSvg /></IconWrap>
                <p className="font-semibold text-gray-900">Ver perfil</p>
              </button>

              {hasActiveRoutine && (
                <button onClick={() => go(`/dashboard/students/${studentId}?tab=rutina`)} className={rowCls}>
                  <IconWrap color="green"><RoutineSvg /></IconWrap>
...
const rowCls = "w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-left hover:bg-gray-50 transition-colors" as const;
...
function IconWrap({ color, children }: { color: string; children: React.ReactNode }) {
  return (
    <div className={`h-7 w-7 rounded-full flex items-center justify-center flex-shrink-0 ${ICON_COLORS[color]}`}>
      {children}
    </div>
  );
}
